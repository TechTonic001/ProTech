// src/pages/admin/AdminLandlords.jsx
import React, { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { adminAPI } from '../../utils/api';
import { formatDate } from '../../utils/formatters';
import LoadingSpinner from '../../components/ui/LoadingSpinner';
import EmptyState from '../../components/ui/EmptyState';
import { 
  Building2, 
  Mail, 
  Phone, 
  MapPin, 
  Calendar,
  ArrowLeft 
} from 'lucide-react';
import toast from 'react-hot-toast';

const AdminLandlords = () => {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(true);
  const [landlords, setLandlords] = useState([]);

  useEffect(() => {
    fetchLandlords();
  }, []);

  const fetchLandlords = async () => {
    try {
      setLoading(true);
      const res = await adminAPI.getLandlords();
      setLandlords(res.data.data || []);
    } catch (err) {
      toast.error(err.message || 'Failed to fetch landlords list');
    } finally {
      setLoading(false);
    }
  };

  if (loading) return <LoadingSpinner fullPage size="lg" />;

  return (
    <div className="max-w-7xl mx-auto space-y-6 animate-fade-in p-2">
      {/* Back to dashboard */}
      <button 
        onClick={() => navigate('/admin/dashboard')}
        className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-900 transition font-bold"
      >
        <ArrowLeft className="w-4 h-4" />
        Back to Dashboard
      </button>

      {/* Header */}
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-2xl lg:text-3xl font-black text-slate-900 tracking-tight">System Landlords</h1>
          <p className="text-xs text-slate-500 font-bold uppercase tracking-wider mt-1">Management of registered landlords and property owners</p>
        </div>
        <div className="bg-slate-100 border border-slate-200 rounded-xl px-3.5 py-1.5 hidden sm:block">
          <span className="text-xs font-bold text-slate-700 uppercase tracking-wider">{landlords.length} Landlords</span>
        </div>
      </div>

      {landlords.length === 0 ? (
        <EmptyState
          title="No Landlords Found"
          description="There are currently no registered landlords on the platform."
          icon={Building2}
          actionLabel="Go to Dashboard"
          onAction={() => navigate('/admin/dashboard')}
        />
      ) : (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-xs overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-slate-50 text-slate-400 font-bold uppercase tracking-wider border-b border-slate-100">
                  <th className="p-4">Landlord Profile</th>
                  <th className="p-4">Contact Info</th>
                  <th className="p-4">Linked Hostel</th>
                  <th className="p-4">Registration Date</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 font-semibold text-slate-650">
                {landlords.map((landlord) => (
                  <tr key={landlord.user_id} className="hover:bg-slate-50/30 transition">
                    {/* Profile */}
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-blue-50 border border-blue-100 text-blue-650 flex items-center justify-center font-bold text-sm">
                          {landlord.full_name?.charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <span className="font-extrabold text-slate-800 text-sm block">{landlord.full_name}</span>
                          <span className="text-[10px] text-slate-400 font-bold uppercase">@{landlord.username}</span>
                        </div>
                      </div>
                    </td>

                    {/* Contact details */}
                    <td className="p-4 space-y-1 text-slate-700 font-medium">
                      <div className="flex items-center gap-1.5">
                        <Mail className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                        <span className="truncate max-w-[180px]">{landlord.email}</span>
                      </div>
                      <div className="flex items-center gap-1.5 text-slate-450">
                        <Phone className="w-3.5 h-3.5 text-slate-350 shrink-0" />
                        <span>{landlord.phone_number || '—'}</span>
                      </div>
                    </td>

                    {/* Hostel Details */}
                    <td className="p-4 space-y-0.5">
                      {landlord.hostel_name ? (
                        <>
                          <div className="font-bold text-slate-800 flex items-center gap-1">
                            <Building2 className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                            {landlord.hostel_name}
                          </div>
                          <div className="text-[10px] text-slate-450 flex items-center gap-1 max-w-[200px] truncate">
                            <MapPin className="w-3.5 h-3.5 text-slate-350 shrink-0" />
                            {landlord.hostel_address}
                          </div>
                        </>
                      ) : (
                        <span className="text-slate-400 italic">No hostel profile linked</span>
                      )}
                    </td>

                    {/* Created date */}
                    <td className="p-4 text-slate-450">
                      <div className="flex items-center gap-1.5">
                        <Calendar className="w-3.5 h-3.5 text-slate-350 shrink-0" />
                        <span>{formatDate(landlord.created_at)}</span>
                      </div>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      )}
    </div>
  );
};

export default AdminLandlords;
