...
const TenantLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (token && user) {
        if (user.role !== 'tenant') {
          throw new Error('This account is not a tenant account');
        }
        login(token, user);
  ...
  return (
    ...
    >
      <div className="font-outfit space-y-6">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Tenant Sign In</h1>
          <p className="text-slate-500 text-sm mt-1">Sign in to pay rent and view lease details</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full px-4 py-3 pl-11 pr-11 border rounded-xl text-sm text-slate-900 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent focus:bg-white transition placeholder:text-slate-400 ${
          ...
          <div className="flex justify-between items-center mt-2">
            <label className="flex items-center select-none cursor-pointer">
              <input
                type="checkbox"
                className="accent-indigo-600 h-4 w-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500 cursor-pointer"
              />
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 rounded-xl font-bold text-sm bg-gradient-to-r from-indigo-600 to-indigo-700 text-white hover:from-indigo-700 hover:to-indigo-800 active:scale-[0.98] transition shadow-lg shadow-indigo-500/25 disabled:opacity-75 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
          >
            {loading ? (
              <>
        ...
        <div className="text-center pt-5 border-t border-slate-100 space-y-3">
          <div className="text-sm text-slate-500 font-medium">
            New tenant?{' '}
            <Link to="/tenant/register" className="font-bold text-indigo-600 hover:text-indigo-700">
              Register here
            </Link>
          </div>
          <div>
            <Link to="/landlord/login" className="text-xs font-bold text-slate-400 hover:text-slate-600 transition">
